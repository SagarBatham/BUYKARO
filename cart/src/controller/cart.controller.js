...
async function addItemtoCart(req, res) {
    ...
    if (!productId || typeof quantity !== 'number') return res.status(400).json({ error: 'productId and numeric qty required' })

    const user = req.user
    if (!user) {
        const existing = inMemoryCart.items.find(i => i.productId === productId)
        if (existing) existing.qty += quantity
        else inMemoryCart.items.push({ productId, qty: quantity })
        return res.status(201).json({ productId, qty: quantity })
    }
    ...
    // respond with the added/updated item
    return res.status(201).json({ productId, quantity })
}

async function updateItemQty(req, res) {
    const { productId } = req.params
    const qty = normalizeQty(req.body)
    if (typeof qty !== 'number') return res.status(400).json({ error: 'numeric qty required' })

    const user = req.user
    if (!user) {
        const item = inMemoryCart.items.find(i => i.productId === productId)
        if (!item) return res.status(404).json({ error: 'Item not found' })
        item.qty = qty
        return res.status(200).json({ productId, qty: item.qty })
    }
    ...
    item.quantity = qty
    await cart.save()
    return res.status(200).json({ productId, qty: item.quantity })
}
...
async function deleteItem(req, res) {
    ...
    const user = req.user
    if (!user) {
        const idx = inMemoryCart.items.findIndex(i => i.productId === productId)
